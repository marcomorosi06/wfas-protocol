...
#define WFAS_MSG_BUSY           "WFAS_BUSY"
...
typedef enum {
    ...
    WFAS_PKT_OTHER,
    WFAS_PKT_BUSY,
    /* Appended last on purpose: the values above keep the numbering they had
     * before this type existed, so already-built firmware is unaffected. */
    WFAS_PKT_MCAST_ENC
} wfas_packet_type;
...
/* 1 if buf is an audio packet carrying WFAS_FLAG_ENCRYPTED, 0 otherwise.
 * Safe to call on any datagram: control messages and malformed input return 0.
 *
 * A receiver that holds a key MUST gate playback on this. The flag lives in the
 * cleartext header, so an attacker can clear it and hand you a forged packet
 * that looks like an ordinary unencrypted stream; if you play whatever is not
 * encrypted, one flipped bit walks straight past the AEAD. Decide once per
 * session whether the stream is encrypted — from your own configuration, or
 * from a beacon whose MAC verified — and from then on drop every audio packet
 * this returns 0 for. Never let the sender make that decision per packet. */
int  wfas_is_encrypted(const uint8_t *buf, size_t len);
