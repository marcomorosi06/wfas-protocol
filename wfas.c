 ...
 *
 * The multicast beacon is one of those messages and needs its own case for the
 * same reason: without it "WFAS_MCAST_ENC;epoch=..." falls through to the magic
 * check and is handed back as audio, so a receiver plays the ASCII as PCM and
 * never sees the beacon that carries the group salt. */
wfas_packet_type wfas_classify(const uint8_t *b, size_t l) {
    ...
    if (eq(b, l, WFAS_MSG_BUSY))             return WFAS_PKT_BUSY;
    if (starts(b, l, WFAS_MSG_MCAST_ENC))    return WFAS_PKT_MCAST_ENC;
...
/* Goes through wfas_classify rather than reading buf[3] directly: the flags
 * byte of a control message is just its fourth character, and several of them
 * happen to have bit1 set ('S' in "WFAS_..." is 0x53), so a bare bit test would
 * call the multicast beacon an encrypted audio packet. */
int wfas_is_encrypted(const uint8_t *buf, size_t len) {
    if (wfas_classify(buf, len) != WFAS_PKT_AUDIO) return 0;
    return (buf[3] & WFAS_FLAG_ENCRYPTED) ? 1 : 0;
}
